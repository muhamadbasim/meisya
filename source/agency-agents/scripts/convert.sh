...
#   hermes       — Hermes Agent skill files (~/.hermes/skills/agency/)
...
# YAML single-quoted scalar. Safe for one-line frontmatter fields.
yaml_quote_string() {
...
convert_hermes() {
  local file="$1"
  local name description slug outfile body division rel
  ...
  rel="${file#$REPO_ROOT/}"
  division="${rel%%/*}"
  [[ -n "$division" && "$division" != "$rel" ]] || division="agency"

  outfile="$OUT_DIR/hermes/skills/agency/${slug}/SKILL.md"
  mkdir -p "$(dirname "$outfile")"

  cat > "$outfile" <<HEREDOC
---
name: ${slug}
description: $(yaml_quote_string "$description")
version: 1.0.0
...
metadata:
  hermes:
    category: agency
    tags: [agency, agency-agents, ${division}]
---

# ${name}

This Hermes skill adapts the Agency Agents "${name}" specialist prompt.
