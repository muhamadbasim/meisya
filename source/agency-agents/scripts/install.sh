...
#   hermes       -- Copy Hermes skills to ~/.hermes/skills/agency/
...
tool_label() {
  case "$1" in
    ...
    hermes)      printf "%-14s  %s" "Hermes"       "(~/.hermes/skills/agency)" ;;
  esac
}
...
install_hermes() {
  local src="$INTEGRATIONS/hermes/skills/agency"
  local dest_root; dest_root="$(resolve_dest hermes "${HOME}/.hermes/skills")"
  local dest="$dest_root/agency"
  local count=0
  ...
  warn "Hermes: start a new Hermes session if these slash commands are not visible immediately."
}
